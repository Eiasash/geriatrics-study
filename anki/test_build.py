...
def test_content_json_exists():
    ...
    assert isinstance(content, list), "content.json should be an array"
...
def test_content_structure():
    ...
    for topic in content:
        assert "topic" in topic, f"Topic missing 'topic' field"
        assert "flashcards" in topic, f"Topic missing 'flashcards' field"
        
        for card in topic.get('flashcards', []):
            assert "q" in card, "Flashcard missing 'q' field"
            assert "a" in card, "Flashcard missing 'a' field"
